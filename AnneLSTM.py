...
def get_a_cell(lstm_size, keep_prob):
    ...
    return drop
# 版权声明：本文为CSDN博主「萤火虫之暮」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
# 原文链接：https://blog.csdn.net/weixin_40759186/article/details/82893626
# ## 3.2 LSTM层
...
class CharRNN:
    ...
    def __init__(self, num_classes, batch_size=100, num_steps=100,
                 ...
                 grad_clip=5, sampling=False, use_embedding=False, embedding_size=128):
...
#                       '训练步数: {}/{}... '.format(counter, batch_size),
#                       '训练误差: {:.4f}... '.format(batch_loss),
#                       '{:.4f} sec/batch'.format((end - start)))
#
#             if (counter % save_every_n == 0):
#                 saver.save(sess, "model/" + name + "/i{}_l{}.ckpt".format(counter, lstm_size))
#         time01 = time0 - time.time()
...
#
# 选用最终的训练参数作为输入进行文本生成
checkpoint = tf.train.latest_checkpoint('model/' + name)
samp = sample(checkpoint, 2000, lstm_size, len(vocab), prime="The ")
print(samp)
#
# # In[22]:
#
# checkpoint = "model/" + name + "/i280_l512.ckpt"
# samp = sample(checkpoint, 1000, lstm_size, len(vocab), prime="Far")
# print(samp)
#
# # In[23]:
#
# checkpoint = 'model/' + name + '/i1700_l512.ckpt'
